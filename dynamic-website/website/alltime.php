...
$mysqli = mysqli_connect ('localhost', '2108418', 'Leslieblantina@1', 'db2108418');
		...
		$res = mysqli_query($mysqli, "SELECT Film_name, Director, Genre, Lead_Actor, Score, image FROM Best WHERE BADGE = 'LEGENDARY';");
		if(!$res) {
		...
		while ($row = mysqli_fetch_assoc($res)){
			echo '<h1>Moviename:</h1>'.$row['Film_name']. " <h1>Director:</h1>" . $row['Director'] ."<br> <h1>Genre:</h1>". $row['Genre'] ." <h1>Lead Actor:</h1>". $row['Lead_Actor']
			." <h1>Score Out of 100:</h1>". $row['Score'].'<h1><br>';
			echo $row['image'];
		}
?>
