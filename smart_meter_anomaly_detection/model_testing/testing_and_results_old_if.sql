create temp table old_flagged_anomalies as
select * from anomalies_isolation_forest;

-- unseen by the model
CREATE TEMP TABLE control_unseen AS
SELECT ml.*
FROM ml_unknown_control_sample_unseen_raw ml
LEFT JOIN old_flagged_anomalies ol
...
SELECT
  ...
  cu.std_or_tou,
  cu.holiday,
  LEAST(st.p01 * 0.1, cu.hourly_mean_reading * 0.01) AS hourly_mean_reading,  -- 90%+ drop
  cu.weather,
  TRUE
...
WHERE cu.weather = 'Freezing'
ORDER BY random()
LIMIT 100;

-- 0 inserted
-- Winter low periods (Dec, Jan, Feb) synthetic anomalies
INSERT INTO synthetic_anomalies
SELECT
  date_time,
  acorn_group,
  std_or_tou,
  holiday,
  hourly_mean_reading * 0.20 AS hourly_mean_reading,  -- 80% drop
  weather,
  TRUE
FROM control_unseen
...
SELECT
  ...
  std_or_tou,
  holiday,
  hourly_mean_reading * 0.30 AS hourly_mean_reading,  -- 70% drop
  weather,
  TRUE
FROM control_unseen
...
SELECT
  ...
  std_or_tou,
  holiday,
  hourly_mean_reading * 3.0  AS hourly_mean_reading,  -- 200% increase
  weather,
  TRUE
FROM control_unseen
WHERE acorn_group IN (
...
SELECT
  ...
  std_or_tou,
  holiday,
  hourly_mean_reading * 2.0  AS hourly_mean_reading,  -- 100% increase
  weather,
  TRUE
FROM control_unseen
...
SELECT
  ...
  std_or_tou,
  holiday,
  hourly_mean_reading * 4.0  AS hourly_mean_reading,  -- 300% increase
  weather,
  TRUE
FROM control_unseen
...
CREATE TABLE test_with_synthetic (
...
-- populate 
INSERT INTO test_with_synthetic
SELECT
  reading_id,
  date_time,
  std_or_tou,
...
SELECT
  640110359 + ROW_NUMBER() OVER (ORDER BY date_time, acorn_group),
  date_time,
  std_or_tou,
  acorn_group,
  holiday,
...
CREATE or replace VIEW synthetic_anomalies_unseen_raw AS
SELECT 
    date_time,
