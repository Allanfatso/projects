//represents our virtual character / non-player character.
//This script will use the movement algorithms and ObjProps to implement movement for the virtual character.

/* Steering Obstacle avoidance parameters
 *
 *AgentProps aAProps – the agent properties.
AgentProps aTargetAProps – the target properties.
ref SteeringOutput SOutput – the output from the algorithm.
float avoidDistance – the distance to try and avoid an obstacle.
float lookahead – the forward look ahead distance.
float lookAheadWhiskers – the distance of the whiskers.
float lookAheadHeightOffset –
the height of the forward and whisker raycasts relative to the the
gameObjects y position.
float lookAheadForwardOffset – the forward offset of the raycasts.
GameObject go – the gameObject for the agent
 *
 */
...
public class Agent_CON : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // ------------------------
        // Steering - Movement algorithms. Comment to make agent wander
       ...
       // movAlg.SteeringSeek_1(myProps, TargetProps, ref SOutput);
        ...
        // Cancel position change if no steering is true. Comment till here */
